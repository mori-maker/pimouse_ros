...
class Motor():
    def __init__(self):
        if not self.set_power(False): sys.exit(1)  #モーターの電源ON set_powerは書込み失敗時にFalseを出すのでシステム終了となる

        rospy.on_shutdown(self.set_power) #ノードが終了したときに実行される
        self.sub_raw = rospy.Subscriber('motor_raw', MotorFreqs, self.callback_raw_freq)
        self.sub_cmd_vel = rospy.Subscriber('cmd_vel', Twist, self.callback_cmd_vel)
        self.srv_on = rospy.Service('motor_on', Trigger, self.callback_on)
        self.srv_off = rospy.Service('motor_off', Trigger, self.callback_off)
        self.str_tm = rospy.Service('timed_motion', TimeMotion, self.callback_tm)
        self.last_time = rospy.Time.now()
        self.using_cmd_vel = False

        
    def set_power(self,onoff=False):  #モーターに通電する
        en = "/dev/rtmotoren0"  #rtmotren0 = 1 でモータに通電
        try:
            with open(en,'w') as f:
                f.write("1\n" if onoff else "0\n")
            self.is_on = onoff
